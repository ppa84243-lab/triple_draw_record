...
PREDRAW_ACTIONS_FACING_RAISE = ["fold", "call", "raise"]
...
def auto_fold_skipped_pre_players(target_player_id):
    ...
    # まだ誰もアクションしていないなら、UTGからtargetの直前までをfold
    if not acted_ids:
        start_index = -1
    else:
        # 最後にアクションした人の位置からtargetまでを見る
    ...
    # 通常の前進だけ自動foldする
    # targetがstartより前にいる場合は、raise後に前の人へ戻っているケースなので自動foldしない
...
def apply_pre_action_for_player(player_id, action):
    ...
    押したプレイヤーより前にいる未入力プレイヤーは自動foldする。
    """
    street = "pre"
    ...
    # 先に、飛ばされた未入力プレイヤーを自動fold
    auto_fold_skipped_pre_players(player_id)
...
# =========================
    ...
    with log_ops[0]:
        if st.button("残り全員fold", key=f"fold_remaining_{current_step}_{street}"):
            fold_remaining_players(street)
            st.rerun()

    with log_ops[1]:
        if st.button("1つ戻す", key=f"undo_action_{current_step}_{street}"):
            undo_action_log(street)
            st.rerun()

    with log_ops[2]:
        if st.button("このbettingクリア", key=f"clear_betting_{current_step}_{street}"):
            clear_action_log(street)
            st.rerun()

    with log_ops[3]:
        if st.button("このbettingリセット", key=f"reset_betting_{current_step}_{street}"):
            clear_action_log(street)
